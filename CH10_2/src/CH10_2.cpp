 ...
 *      Author: 정회윤
 *
 *  + PersonManager 내의 persons 멤버를 Vector 대신 vector 클래스 사용하기
 *  + auto 사용
 *  + BasePM 클래스 추가: PersonManager의 기존 display(), copyPersons() 등
 *                     기존 함수 BasePM으로 옮기기
 *  + CppSTL 클래스 추가: 1.Display 2.Shuffle 3.Sort 4.ReverseSort 5.Reverse
 *  + PersonManager, CppSTL에서 BasePM 상속한 후 
 *    기존 display(), copyPersons() 함수 삭제하기
*/
...
Student::Student(const Student& s) :
	Person(s), department(s.department), GPA(s.GPA), year(s.year)
{
