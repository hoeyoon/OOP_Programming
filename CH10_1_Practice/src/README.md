...
아래의 mapFind()는 6번 메뉴을 선택했을 때 실행되는 함수이다. 
1) 아래 mapFind(), find_map()의 코드를 소스 파일에 복사하라. 
    find_map()은 난수로 발생시킨 키를 mp에서 검색한 후 검색 결과를 출력한다.
2) find_map()의 TODO 부분을 구현하라.
```
```c++
void find_map() {
    int key = rnd();
...
### 문제 7 설명
```
아래의 mapToVector()는 7번 메뉴을 선택했을 때 실행되는 함수이다.
1) 아래 mapToVector()의 코드를 소스 파일에 복사하라. 
    이 함수는 벡터 전체를 정렬한 후 정렬된 벡터를 출력한다. 
    이때 정렬하는 알고리즘을 사용하지 말고 정렬해 보라.
2) 이 함수의 TODO 부분을 구현하라.
```
```c++
void mapToVector() {
